...
class CheckVariablesCommand(gdb.Command):
    ...
    def load(self, json_path:str):
        ...
        cmd = "readelf -s {} | grep ' {}$' | awk '{{print $2}}' | head -n 1".format(exe_path, ref_func)
        addr_str = os.popen(cmd).read().strip()
        ref_static_addr = int(addr_str, 16)
        # gdb.execute("start")
        ...
        for var in json_list:
            if var["expression"] == "" or "." in var["name"] or "->" in var["name"]:
                continue
    ...
    def invoke(self, args, from_tty):
        ...
        def get_pc():
          ...
          # convert rbp + rsi + offset to long(rbp) + long(rsi) + (long)offset
        def convert_expression(expression:str) -> str:
            token_pattern = re.compile(r'(\$[a-zA-Z0-9]+|\b\d+\b)')
    
            def replacer(match):
                token = match.group(0)
                return f"(long){token}"
            converted_expr = token_pattern.sub(replacer, expression)
        ...
        def get_candidate_values(var:dict, type_str:str) -> set:
            ...
            for name in candidate_names:
                if "xmm" in name:
                    ...
                    candidate_values.append(int(gdb.parse_and_eval(f"({type_str})({convert_expression(name)}.v2_int64[0])")))
                
                else:
                    candidate_values.append(int(gdb.parse_and_eval(f"({type_str})({convert_expression(name)})")))

            return set(candidate_values)

        # break at all match positions
        for addr in self.addrs:
            #if addr in self.arrived_addr:
                #print(f"skip {addr:X}")
                #continue
        ...
        args_list = shlex.split(self.option) + [self.input_file]
        args_str = " ".join(args_list)
        gdb.execute(f"set args {args_str}")  # 先设置参数
        gdb.execute("run")                  # 再运行
