...
int parse_simple_ranges(Dwarf_Die die, Dwarf_Addr *startpc, Dwarf_Addr *endpc) {
  ...
  */
  ...
  // low pc, Given DIE and attribute number return a Dwarf_attribute
  ...
  if (low_form == DW_FORM_addr) {
    // set `startpc` Return the address when the attribute has form address.
  ...
  /*
      ...
      if of class constant , it's the offset from start_pc
  */
...
Range::Range(const Range &range) : startpc(range.startpc), endpc(range.endpc) {}

Range Range::createEmpty() {
  Range res;
  res.startpc = 0;
  res.endpc = 0;
  return res;
...
Range Range::createFromDie(Dwarf_Die die) {
  std::cout << "in create From die"
            << "\n";
...
void Range::setFromRange(const Range &range) {
